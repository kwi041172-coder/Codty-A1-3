...
- **AI API**: Codyssey Anthropic 호환 API의 Claude Haiku 4 (`anthropic>=1.0.0,<2.0.0`)
- **Deployment**: Vercel
...
Anthropic SDK는 `https://copa.codyssey.kr/v1/messages`를 사용하도록 `ANTHROPIC_BASE_URL`을 설정합니다.

## 3. 환경 변수 설정
Vercel 프로젝트 설정에서 아래 키를 반드시 추가해야 합니다:
- `ANTHROPIC_API_KEY`: Codyssey에서 발급받은 Anthropic 호환 virtual-key
- `ANTHROPIC_BASE_URL`: `https://copa.codyssey.kr` (선택 사항, 기본값)
- `ANTHROPIC_MODEL`: `claude-haiku-4` (선택 사항, 기본값)

## 4. 실행 방법
1. 이 저장소를 Clone 합니다.
2. `requirements.txt`에 명시된 패키지를 설치합니다 (`pip install -r requirements.txt`).
3. Vercel 프로젝트 환경 변수에 `ANTHROPIC_API_KEY`를 등록합니다. 필요하면 `ANTHROPIC_BASE_URL`과 `ANTHROPIC_MODEL`도 등록합니다.
...
### Claude 인증 오류 점검
- `ANTHROPIC_API_KEY`에는 Codyssey에서 발급받은 virtual-key를 입력합니다.
...
- 모델을 변경해야 하면 Vercel 환경 변수에 `ANTHROPIC_MODEL`을 추가할 수 있습니다. 기본값은 `claude-haiku-4`입니다.
- Codyssey의 모델별 차감 배수, 사용 가능 상태, 계정 한도를 확인합니다.
